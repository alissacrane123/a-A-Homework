...
def top_titles
  ...
  movie = Movie.select([:id, :title]).where(score: 9 )
...
def below_average_years
  ...
  Movie.select(['COUNT(title)', :yr]).where('score < 5').group(:yr).order('COUNT(title) DESC').count('title')
end
...
def pulp_fiction_actors
  ...
  Actor.select([:id, 'DISTINCT name']).joins(:castings).joins(:movies).where("title LIKE 'Pulp Fiction'")
end
